...
var global_TAU23=0.0;
var global_Y_T_is=0.0;
var global_S_L_is=0.0;
...
global_alpha=coletar_alpha(52.3);
global_TAU23=coletar_TAU23(TAU12);
global_Y_T_is=coletar_Y_T_is(1.12*Math.sqrt(2)*SIGMA_T_2);
global_S_L_is=coletar_S_L_is(Math.sqrt(2)*TAU12);
...
//Modos do IF
//	Modo 1, 
//Compressão na Matriz	Muito parecido com um circulo de Mohr
//Caso do Termo T	

	if_larc_03_compressao_matriz_larc0301=0.0;
	if(sigma_2<0)
	{
//melhora USAR SIGMA_C_2
		if(sigma_2<=sigma_1)
		{
...
//alpha=alpha_0;
			...
			tau_eff_L=0.0;
			previo=Math.cos(alpha)*(Math.abs(tau_12)+eta_L*sigma_2*Math.cos(alpha));
			if(previo>0)
			{
				tau_eff_L=previo;
			}
...
//Modo 4, Compressão na Fibra, Depende do valor de sigma_2_m
	...
	fi_angulo=0.0;
	fi_angulo=calcular_Fi(sigma_1,sigma_2,tau_12,theta,SIGMA_T_1,SIGMA_T_2,SIGMA_C_1,SIGMA_C_2,TAU12);


	theta_radianos=(Math.PI/180)*fi_angulo;
	c=Math.cos(theta_radianos);
	s=Math.sin(theta_radianos);
	c2=c*c;
	s2=s*s;
...
//S_L_is=Math.sqrt(2)*TAU12;
			alpha_0=53.2*(Math.PI/180);
			eta_L=(TAU12*Math.cos(alpha_0*2))/(SIGMA_C_2*Math.pow(Math.cos(alpha_0),2));
			previo=(Math.abs(tau_12_m)+eta_L*sigma_2_m)/(S_L_is);
...
if(refinamento=="bici")
{
	alert("radicando  "+typeof(radicando)+"  "+radicando+"   fi_angulo "+fi_angulo);
}
*/
//########################################################################################################################################################################
//########################################################################################################################################################################
...
//alert(j+") INICIO Nov07 IF.length  "+IF[0]+"   "+IF[1]+"   "+IF[2]+"   "+IF[3]+"   "+IF[4]+"   "+IF[5]);
...
Coletar
*/
function coletar_alpha(alpha_entrada){
    var entrada = prompt("Por favor indique o ângulo alpha (Degree)", alpha_entrada);
    if (entrada != null) {
	return entrada;
    }
    else
    {
	 return alpha_entrada;
    }	
}
//////////////////////////////////////////////////////////
function coletar_TAU23(TAU23_entrada){
    var entrada = prompt("Por favor indique o TAU12 (Pa)", TAU23_entrada);
    if (entrada != null) {
	return entrada;
    }
    else
    {
	 return TAU23_entrada;
    }	
}
//////////////////////////////////////////////////////////
function coletar_Y_T_is(Y_T_is_entrada){
    var entrada = prompt("Por favor indique o Y_T_is", Y_T_is_entrada);
    if (entrada != null) {
	return entrada;
    }
    else
    {
	 return Y_T_is_entrada;
    }	
}
//////////////////////////////////////////////////////////
function coletar_S_L_is(S_L_is_entrada){
    var entrada = prompt("Por favor indique o S_L_is", S_L_is_entrada);
    if (entrada != null) {
	return entrada;
    }
    else
    {
	 return S_L_is_entrada;
    }	
}
//////////////////////////////////////////////////////////
